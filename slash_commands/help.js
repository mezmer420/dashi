...
module.exports.run = async ({ client, interaction }) => {
	...
	const infoEmbed = new MessageEmbed()
		...
		.addFields(
			{
				name: "For Everyone",
				value: "`!bigcock`  `!cockroach`  `!emojify [message]`  `!good`",
			},
			{
				name: "For Government",
				value: "`!gulag`",
		...
		.addField(
			"**Slash Commands**",
			"I have over 50 slash commands that everyone can use + government commands. Type `/` and click me to view my slash commands available to you in your current channel. If you're an admin, mezmer has hardcoded certain commands such that only he can use them. ;)"
		)
		.setFooter({
			text: `${mezmer.tag}`,
