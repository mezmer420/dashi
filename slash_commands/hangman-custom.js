...
module.exports.run = async ({client, interaction}) => {
    if(interaction.channel.id == "965054741480636496" || interaction.channel.id == "950196454880866314"){
        const word = interaction.options.getString("word")
        const targetchannel = interaction.options.getChannel("channel")
        const category = interaction.options.getString("category")

        if(targetchannel.id !== "939674946953682976" && targetchannel.id !== "939686071241949205"  && targetchannel.id !== "940786577808969738" && targetchannel.id !== "945527434655187006" && targetchannel.id !== "969027553878749204"){
            return await interaction.editReply({
                content: "You can only set the target channel to <#939674946953682976>, <#939686071241949205>, <#940786577808969738>, <#945527434655187006>, or <#969027553878749204>"
            })
            .catch((err) => {
                return
            })
        ...
        await client.channels.cache.get(`${targetchannel.id}`).send(content2)
        .catch((err) => {
        ...
        return await interaction.editReply({ 
            content: "You can only use this command in <#965054741480636496> and <#950196454880866314>"
        })
        .catch((err) => {
            return
        })
