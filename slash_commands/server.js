	...
	switch (Options) {
		case "info": {
			...
			const publicthreads = await guild.channels.cache.filter(
				(ch) => ch.type == "GUILD_PUBLIC_THREAD"
			).size
			...
			let levelinfo = `Level ${boostlevel.replace("TIER_", "")}`
			...
			const Embed = new MessageEmbed()
				...
				.addFields(
					...
					{
						name: "Channels",
						value: `${totalchannels} total (not counting threads) in ${categories} ${CategoryorCategories}:\n${textchannels} text channels\n${voicechannels} voice channels\n————————\n${publicthreads} public ${ThreadorThreads}`,
						inline: true,
					},
					{
						name: "Emojis",
